...
# Having last minute issues with this as a discrete class so abandoning that for right now
class MovementCurve:
    ...
    def __init__(self, curve_type, axis, length, peak, ax = None, curve_shape = Curve_Shape.DEFAULT, frequency = 1):
        ...
        self.curve_yvals = self.curve_plot.lines[config.curve_index].get_ydata()
        self.curve_plot.get_lines()[config.curve_index].set_linestyle(self.line_type_from_curve_type())
